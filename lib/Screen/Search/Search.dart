...
class _SuggestionList extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Separate history and non-history suggestions
    final List<Product> historySuggestions = suggestions!.where((s) => s.history == true).toList();
    final List<Product> gridSuggestions = suggestions!.where((s) => s.history != true).toList();

    return Column(
      children: [
        // Display history suggestions as a list
        if (historySuggestions.isNotEmpty)
          Container(
            height: 0, // Set a height for the list view
            child: ListView.builder(
              scrollDirection: Axis.vertical,
              itemCount: historySuggestions.length,
              itemBuilder: (BuildContext context, int i) {
                final Product suggestion = historySuggestions[i];
                return GestureDetector(
                  ...
                  child: Container(
                    width: 150, // Set width for list items
                    ...
                    child: Column(
                      ...
                      children: [
                          ...
                          child: ClipRRect(
                            ...
                            child: suggestion.image == ''
                              ...
                              fit: BoxFit.cover,
                                ...
                                : DesignConfiguration.getCacheNotworkImage(
                              imageurlString: suggestion.image ?? 'default_placeholder_url',
                              boxFit: BoxFit.cover,
                              context: context,
                        ...
                        SizedBox(height: 4.0),
                        Text(
                          'AED${suggestion.prVarientList?.first.price}', // Display the price with the Indian Rupee symbol
                          style: Theme.of(context).textTheme.titleSmall!.copyWith(
        ...
        // Display other suggestions as a grid
        if (gridSuggestions.isNotEmpty)
          Expanded(
            child: GridView.builder(
              itemCount: gridSuggestions.length,
              shrinkWrap: true,
              ...
              itemBuilder: (BuildContext context, int i) {
                final Product suggestion = gridSuggestions[i];
                return GestureDetector(
                  onTap: () async {
                    if (suggestion.history == true) {
                      clearAll?.call();
                      if (suggestion.name != null) {
                        textController?.text = suggestion.name!;
                        textController?.selection = TextSelection.fromPosition(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 4.0),
                          Text(
                            'AED ${suggestion.prVarientList?.first.price}',
