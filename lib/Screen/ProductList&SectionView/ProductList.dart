...
//  List<Product> tempList = [];
  ...
  Widget _tags() {
    ...
    var subListGroup = HomePageProvider.catListTemp;
    String? catCommonId;

    if(this.productList.length > 0 )
      {
       catCommonId= this.productList[0].categoryId;

       var subItemTemp = subListGroup
           .toList()
           .where((x) => x.id != null && x.id == catCommonId)
           .toList();

       if(subItemTemp.length > 0 ) {
         var tempitems = subItemTemp[0].subList;

         tagList = [];
         tagImageList=[];
         // Add all names to tagList
         // Using forEach to add names to tagList
         tempitems?.forEach((item) {
           tagList?.add(item.name.toString());
           tagImageList?.add(item.image.toString());
         });

       }
       else
         {
           var subItemTemp_1 = subListGroup
               .toList()
               .where((x) => x.subList != null)
               .toList();


           subItemTemp_1?.forEach((itemSub) {

             var subItemTemp = (itemSub.subList ?? [])
                 .where((x) => x.id != null && x.id == catCommonId)
                 .toList();
    ...
    if (widget.tag != null && !widget.tag!) {
      if (tagList != null && tagList!.isNotEmpty) {
        // Ensure the tagList is sorted if necessary (can be removed if order doesn't change)
        //tagList!.sort();

        return Container(
          height: 50, // Height of the container holding the ListView
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return Padding(
                  ...
                  child: ChoiceChip(
                    key: ValueKey(tagList![index]), // Stable key based on tag value
                    selected: false,
                    label: Row(
