...
/*     _controller1.addListener(
          ...
          child: Column(
            ...
            children: [
              SizedBox(
                width: deviceWidth,
                child: Text(
                  getTranslated(context, 'Customer Comments'),
                  style: TextStyle(
  ...
  // Add this method inside your StateItem class
  void handleChipSelection(int attributeIndex, int chipIndex) {
    if (mounted) {
      setState(() {
        // Update the selected variant
        widget.model!.selVarient = chipIndex;
        _oldSelVarient = chipIndex;
        available = false;
        _selectedIndex[attributeIndex] = chipIndex;

        // Collect selected attribute IDs
        List<int> selectedIds = [];
        for (int i = 0; i < widget.model!.attributeList!.length; i++) {
          List<String> attIds = widget.model!.attributeList![i].id!.split(',');
          if (_selectedIndex[i] != null) {
            selectedIds.add(int.parse(attIds[_selectedIndex[i]!]));
          }
        }

        // Match the selected IDs with product variants
        List<bool> check = [];
        varSelected = null;

        for (int i = 0; i < widget.model!.prVarientList!.length; i++) {
          List<String> sinId = widget.model!.prVarientList![i].attribute_value_ids!.split(',');

          if (selectedIds.every((id) => sinId.contains(id.toString()))) {
            check.add(true);
            if (selectedIds.length == sinId.length && check.length == selectedIds.length) {
              varSelected = i;
              selectIndex = i;
              break;
            }
        ...
        if (selectedIds.length == (varSelected != null ? widget.model!.prVarientList![varSelected!].attribute_value_ids!.split(',').length : 0) &&
            check.length == selectedIds.length) {
        ...
        // Update the product image view
        updateImageView();
      });
    }
  }

// Helper method to update variant availability
  void updateVariantAvailability() {
    if (widget.model!.stockType == '0' || widget.model!.stockType == '1') {
...
// Helper method to update the product image view based on selected variant
  void updateImageView() {
    if (widget.model!.prVarientList![_oldSelVarient].images!.isNotEmpty) {
      ...
      int? p;
      if (widget.model!.videType != '' && widget.model!.video!.isNotEmpty) {
        p = widget.model!.otherImage!.length + 2 + oldVarTotal;
      } else {
        p = widget.model!.otherImage!.length + 1 + oldVarTotal;
      }
  ...
  getvariantPart() {
    ...
    if (widget.model?.attributeList?.isEmpty ?? true) {
      // If attribute list is null or empty, return SizedBox
      return const SizedBox();
    }
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            List<Widget> chips = [];
            var attribute = widget.model!.attributeList![index];

            // Ensure non-null and non-empty values
            List<String> att = attribute.value?.split(',') ?? [];
            List<String> attId = attribute.id?.split(',') ?? [];
            List<String> attSType = attribute.sType?.split(',') ?? [];
            List<String> attSValue = attribute.sValue?.split(',') ?? [];
            List<String> wholeAtt = widget.model?.attrIds?.split(',') ?? [];

            // Check for matching index in _selectedIndex
            int? selectedIdx = (_selectedIndex.length > index) ? _selectedIndex[index] : null;

            for (int i = 0; i < att.length; i++) {
              if (i >= attSType.length || i >= attSValue.length) continue; // Avoid out-of-bounds errors
              ...
              if (attSType[i] == '1') {
                ...
                itemLabel = Container(
                  ...
                  decoration: BoxDecoration(
                    shape: BoxShape.circle,
                    color: selectedIdx == i ? colors.primary : colors.secondary,
                  ),
                  child: Center(
                    child: Container(
                      width: 25,
              ...
              // Add chip to list if valid
              if (selectedIdx != null && wholeAtt.contains(attId[i])) {
                chips.add(
                  Padding(
